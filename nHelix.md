...
Sur Mac, il est possible d’installer LemMinx avec Maven (qui peut s’installer avec Brew). Toutefois, après l’installation l’implémentation est en partie dysfonctionnelle (doublement des chevrons lors de la frappe d’un élément).

##### Utiliser l’extension Red Hat XML de Visual Studio code

L’extension Red Hat XML de Visual Studio code semble bien mieux fonctionner. Celle-ci se trouve normalement dans `~/.lemminx/`. Mais quand si Java est disponible sur l’ordinateur, elle peut être située dans quelque chose comme : `~/.vscode/extensions/redhat.vscode-xml-*/server/`

```bash
find ~/.vscode/extensions -name "lemminx*" 2>/dev/null
```

Récupérer le chemin

```bash
ls ~/.vscode/extensions/redhat.vscode-xml-*/server/
```

Créer un lien symbolique dans /usr/local/bin/

```bash
sudo ln -sf ~/.vscode/extensions/redhat.vscode-xml-*/server/lemminx-osx-aarch_64 /usr/local/bin/lemminx
```

##### Installer LemMinx avec Maven

Pour installer LemMinx avec Maven, après avoir installé Maven avec Brew, créer un fichier poml
...
Relancer avec les dépendances 

```bash
mvn dependency:copy-dependencies -DoutputDirectory=$HOME/lemminx-jar
```
...
```bash
java -jar ~/lemminx-jar/org.eclipse.lemminx-0.28.0-uber.jar &
```

Arrêter avec `kill %1`

```bash
sudo hx /usr/local/bin/lemminx
```
...
```sh
#!/bin/sh
exec java -jar /Users/emmanuelchateau/lemminx-jar/org.eclipse.lemminx-0.28.0-uber.jar "$@"
```
...
##### Configuration dans `languages.toml`
...
Commandes LSP. Dans Helix, les commandes LSP sont accessibles via ces raccourcis en mode normal :
...
- `K` — hover (documentation sur l'élément sous le curseur)
- `<space>d` — diagnostics du fichier courant
- `<space>D` — diagnostics de tout le workspace

**Complétion**

- `<Ctrl-x>` — déclencher la complétion manuellement (en mode insertion elle s'active automatiquement)

**Actions**

- `<space>a` — code actions (suggestions de corrections)
- `<space>r` — renommer un élément
- `=` — formater le fichier (ou la sélection)

**Symboles**

- `<space>s` — symboles du fichier courant
- `<space>S` — symboles du workspace

Pour XML spécifiquement, les plus utiles au quotidien seront `K` pour inspecter un élément ou attribut, `<Ctrl-x>` pour la complétion des balises et attributs, `=` pour formater, et `<space>d` pour voir les erreurs de validation.
...
Il est possible de configurer Helix pour exécuter des blocs de code. La fonctionnalité n’est pas nativement disponible avec Helix, toutefois il est possible d’utiliser les splits de Ghostty ou bien Tmux pour le multifenestrage. On peut ajouter une commande pour exécuter un script qui enverra à Julia le code à exécuter.

- Faire un split avec Ghostty (Cmd+D)
- Exécuter Julia dans le terminal ouvert `julia` (nous avons configurer Julia avec un startup.jl pour charger les environnements)
- sélectionner le code à exécuter en mode Sélection (v) ou sélection d’une ligne avec x en mode normal
- exécuter Space + l pour envorer le code au REPL Julia

- Ghostty comme terminal offre des splits natifs (Cmd+D)
- Helix avec le LSP Julia pour l’autocomplétion
- Space + l en mode sélection pour envoyer du code au REPL Julia
startup.jl qui active automatiquement l’environnement du projet

Configuration

Créer un script `~/.local/bin/send-to-ghostty.sh`
...
on run argv
    set codeFile to item 1 of argv
    set fileHandle to open for access POSIX file codeFile
    set codeText to read fileHandle
    close access fileHandle
    tell application "Ghostty"
        set t to terminal 2 of selected tab of front window
        input text codeText to t
        send key "enter" to t
    end tell
end run
EOF
...
Ajouter à `~/.config/helix/config.toml`

```bash
[keys.select]
space.l = [":pipe-to send-to-ghostty.sh"]

[keys.normal]
space.l = ["extend_line_below", ":pipe-to send-to-ghostty.sh"]
```
